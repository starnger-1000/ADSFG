...
async def finalize_auction(item_type: str, item_id: str, channel_id: int):
    ...
    if winner:
        ...
        if "(group)" in bidder_str:
            ...
            bidder_user_id = db.fetchone("SELECT user_id FROM personal_wallets WHERE user_id LIKE ?", (str(bidder_str),)) # Simple personal bidder
            if bidder_user_id:
                db.query("UPDATE personal_wallets SET balance=balance-? WHERE user_id=?", (amount, bidder_user_id['user_id']))
        ...
        if item_type == "club":
            ...
            if club:
                if gname: # Group ownership
                    group_owner_id = db.fetchone("SELECT owner_id FROM investor_groups WHERE name=?", (gname,))
                    db.query("UPDATE club SET owner_id=? WHERE id=?", (group_owner_id['owner_id'], club['id']))
...
# Remove depositwallet command (as per point 6)
# @bot.command()
...
async def adjustsalary(ctx, duelist_id: int, amount: int):
    ...
    club_owner_str = contract["club_owner"]
...
async def admin_reset_all(ctx):
    ...
    db.query("UPDATE club SET total_wins=0, level_name='Unranked', value=base_price, owner_id=NULL, last_bid_price=NULL")
...
async def deleteclub(ctx, club_name: str):
    ...
    # Note: More cleanup (bids, owner_profiles, etc.) might be needed in a production environment
