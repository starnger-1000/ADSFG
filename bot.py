...
# --- NEW: Market Panel Command (Dashboard View) ---
@bot.command()
async def marketpanel(ctx, club_name_or_id: str):
    ...
    embed = discord.Embed(
        ...
        description=f"Current Division: **{current_level}**", 
        color=0xf1c40f
    )
    ...
    embed.add_field(name="Value Change/Battle", value=f"+/-{WIN_VALUE_BONUS:,}", inline=True)
    
    # Level Up Progression
    if next_level_info:
        embed.add_field(name="Next Division", value=f"**{next_level_info[0]}**", inline=True)
        embed.add_field(name="Wins Needed", value=f"**{required_wins}** more wins", inline=True)
        embed.add_field(name="Level Bonus", value=f"**{next_level_info[2]:,}**", inline=True)
    else:
        embed.add_field(name="Next Division", value="**MAX LEVEL REACHED**", inline=False)
        
    embed.set_footer(text="Market value fluctuates hourly and changes with every battle result.")
...
# NOTE: Removed duplicate definition of deleteclub here, ensuring it is only defined once.
...
async def helpme(ctx):
    ...
    # --- MESSAGE 1/3: General & Wallet ---
    help_message_1 = f"""
💰 **ZE AUCTION BOT - HELP (1/3) General & Wallet** 💰

The current prefix is: `{current_prefix}`.

**[ PERSONAL / WALLET ]**
- `{current_prefix}profile [ @User ]`
  -> Shows your profile, owned club, share %, and recent bids.
- `{current_prefix}wallet`
  -> Shows your current personal cash balance.
- `{current_prefix}withdrawwallet <Amount>`
  -> Withdraws cash from your personal wallet.
- `{current_prefix}retireduelist`
  -> Removes your duelist registration (if uncontracted).
...
- `{current_prefix}joingroup <Name> <Share %>`
  -> Joins an existing group and buys a share percentage (must not exceed 100% total).
- `{current_prefix}leavegroup <Name>`
  -> Leaves a group (must sell shares first).
- `{current_prefix}deposit <Group Name> <Amount>`
  -> Funds the group wallet for group bidding (only way to deposit).
- `{current_prefix}withdraw <Group Name> <Amount>`
  -> Withdraws funds from the group wallet.
"""
    await ctx.send(f"```{help_message_1}```")

    # --- MESSAGE 2/3: Auction, Market & Battles ---
    help_message_2 = f"""
⚔️ **ZE AUCTION BOT - HELP (2/3) Auction, Market & Battles** ⚔️
...
- `{current_prefix}sellclub <Club Name>`
  -> Solo owner sells their club for its current live market value.
- `{current_prefix}sellshares <Club Name> <@Buyer> <Share %>`
  -> Sells your percentage of a group-owned club to another user.

**[ MARKET & LEVEL UP PANEL ]**
...
- `{current_prefix}registerbattle <Club A Name> <Club B Name>`
  -> (Admin only) Registers a new official battle between two clubs.
- `{current_prefix}battleresult <Battle ID> <Winner Club Name>`
  -> (Admin only) Records the result, which updates market value and club wins/levels.
"""
    await ctx.send(f"```{help_message_2}```")

    # --- MESSAGE 3/3: Admin & Manager Commands ---
    help_message_3 = f"""
🛠️ **ZE AUCTION BOT - HELP (3/3) Admin & Manager** 🛠️

The current prefix is: `{current_prefix}`.

**[ ADMIN & OWNER TOOLS ]**
- `{current_prefix}setprefix <New Prefix>`
  -> (Admin only) Dynamically changes the bot's command prefix (e.g., to $).
- `{current_prefix}registerclub <Name> <Base Price> [Logo URL] [Slogan]`
  -> (Admin only) Registers a new club for auction.
- `{current_prefix}startclubauction <Club Name>`
  -> (Admin only) Starts the auction for a club.
- `{current_prefix}startduelistauction <Duelist ID>`
  -> (Admin only) Starts the auction for a duelist.
- `{current_prefix}deleteclub <Club Name>`
  -> (Admin only) Permanently removes a club and all associated history.
- `{current_prefix}checkclubmessages <Club Name> <Count>`
  -> (Admin only) Manually checks owner messages to increase MV.
- `{current_prefix}tip <@User> <Amount>`
  -> (Admin only) Adds balance to a user's personal wallet.
- `{current_prefix}deduct_user <@User> <Amount>`
  -> (Admin only) Deducts balance from a user's personal wallet.
- `{current_prefix}adjustgroupfunds <Group Name> <+/- Amount>`
  -> (Admin only) Manually adjusts a group's fund balance.
- `{current_prefix}forcewinner <Type> <ID> <Winner> <Amount>`
  -> (Owner only) Force sets a winner for an item.
- `{current_prefix}freezeauction / {current_prefix}unfreezeauction`
  -> (Owner only) Stops/starts all bidding activity.
- `{current_prefix}auditlog [Lines]`
  -> (Owner only) Shows recent bot activity.
- `{current_prefix}admin_reset_all`
  -> (Owner only) Resets all club history, wins, and market data.

**[ MANAGER / SALARY ]**
- `{current_prefix}setclubmanager <Club Name> <@Member>`
  -> (Admin only) Assigns a manager to a club.
- `{current_prefix}adjustsalary <Duelist ID> <+/- Amount>`
  -> (Owner only) Gives a bonus (deducted from owner's wallet) or deducts salary from the duelist's wallet.
- `{current_prefix}deductsalary <Duelist ID> <yes|no>`
  -> Deducts 15% of salary (for missing match), taken from club owner funds.
"""
    await ctx.send(f"```{help_message_3}```")
