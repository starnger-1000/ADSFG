    ...
    if winner:
        ...
        if item_type == "club":
            ...
            if duelist:
                ...
                # Check if user specified a target club? No, we check ownership.
                # If user has 1 club, assign. If group, assign.
...
async def placebid(ctx, amount: int, item_type: str, item_id: int, club_name: str = None):
    ...
    if item_type == 'duelist':
        ...
        # Verify ownership of that club
        c = db.fetchone("SELECT * FROM club WHERE name=?", (club_name,))
        if not c:
             return await ctx.send(f"Club **{club_name}** not found.")
        
        if str(ctx.author.id) != c['owner_id']:
             return await ctx.send(f"🚫 You do not own **{club_name}**. You cannot bid for a club you don't own.")
...
async def groupbid(ctx, group_name: str, amount: int, item_type: str, item_id: int, club_name: str = None):
    ...
    if item_type == 'duelist':
        ...
        # Verify group ownership of that club
        c = db.fetchone("SELECT * FROM club WHERE name=?", (club_name,))
        if not c:
             return await ctx.send(f"Club **{club_name}** not found.")
        
        if c['owner_id'] != f"group:{group_name.lower()}":
             return await ctx.send(f"🚫 Group **{group_name}** does not own **{club_name}**.")
...
async def sellclub(ctx, club_name: str):
    ...
    # CONFIRMATION STEP
    await ctx.send(f"⚠️ **SELL CLUB CONFIRMATION**\n{ctx.author.mention}, are you sure you want to sell **{club_name}** for its current market value of **{market_value:,}**? This action cannot be undone.\nType `yes` or `no`.")
    ...
    try:
        ...
        return await ctx.send("Sale timed out. Action cancelled.")
